	...
	const Vector3& size, const Vector3& pos, ColliderType colType)
{
	...
	_animNodes[JUMP00] = make_unique<JumpNode>(this);
	_animNodes[FALL] = make_unique<FallNode>(this);
	...
	_isOnGround = [&](const XMVECTOR& vec)
	{
		...
		return CheckGroundAndSnap(objsNearby);
	};
...
FbxActor::GetContinuousOnGround(const Vector3& dir,	const Vector3& currentPos,
	const float speed)
{
	// 周囲のオブジェクトを取得 (定数はマジックナンバー化を避けるのが理想)
	auto objsNearby = OcTree::Instance().GetByVector(currentPos, dir, 10);

	for (const auto& obj : objsNearby)
	{
		...
		{
			// 統合したロジックで接地・補正を行う (単一要素のリストとして渡す)
			if (CheckGroundAndSnap({ obj })) {
				return true;
			}
		}
	}
...
/// </summary>
/// <param name="objsNearby">判定対象のオブジェクトリスト</param>
/// <returns>接地しているか</returns>
bool FbxActor::CheckGroundAndSnap(const std::vector<std::shared_ptr<IFbx>>& objsNearby)
{
	for (const auto& obj : objsNearby)
	{
		if (!obj || !obj->Collider()) continue;

		// 判定1: 下方向への衝突ベクトル確認 (75.0f〜150.0fの間、より厳密な75.0fを基準に統合)
		if (CollisionDetector::Instance().CheckColAndVector(
			obj->Collider(), *_fbxComp->Collider()->Center(), Vector3(0, -1, 0), 75.0f))
		{
			// BoxColliderであることを安全に確認
			auto boxCol = std::dynamic_pointer_cast<BoxCollider>(obj->Collider());
			if (!boxCol) continue;

			// 地面の上面 = 中心 + 高さの半分
			const float groundTopY = boxCol->Center()->Y() + boxCol->HalfLength()[1];

			// アニメーションによるオフセット調整
			float animationOffset = 0.0f;
			if (!IsAnimationEqual(JUMP00))
			{
				animationOffset = _fbxComp->MappedMats()[1].r[3].m128_f32[1];
			}

			// 座標の補正
			_fbxComp->TransrateVector().Y() = groundTopY - animationOffset;
...
FbxActor::Update()
{
	...
	ImGuiManager::Instance().AddLabelAndBool("OnGround", GetOnGround());
	ImGuiManager::Instance().AddLabelAndVector3("Player",CurrentPosition());
	...
	if (_isInBlend)	copy(_blendMats.begin(), _blendMats.end(), _fbxComp->MappedMats() + 1);
	else copy(_boneMats.begin(), _boneMats.end(), _fbxComp->MappedMats() + 1);
...
/// 入力ベクトルを返す関数
/// </summary>
/// <returns>入力ベクトル</returns>
Vector3 
FbxActor::GetInputVec()const
{
	return _inputVec;
}
...
void
FbxActor::SetRejectBone(bool val)
{
	_fbxComp->SetRejectBone(val);
}
